...
SAFE_TERMS_FILE = "safe_terms.txt"

def load_safe_terms():
    """Load dynamically learned financial terms from file."""
    if not os.path.exists(SAFE_TERMS_FILE):
        return set()
    with open(SAFE_TERMS_FILE, "r", encoding="utf-8") as f:
        return set(line.strip().lower() for line in f if line.strip())

def save_safe_term(term):
    """Add a new safe term to file (avoiding duplicates)."""
    term = term.lower().strip()
    if not term:
        return
    current_terms = load_safe_terms()
    if term not in current_terms:
        with open(SAFE_TERMS_FILE, "a", encoding="utf-8") as f:
            f.write(term + "\n")
        print(f"💾 Added new safe term: {term}")

# Load dynamic safe terms at startup
DYNAMIC_SAFE_TERMS = load_safe_terms()
...
def poll_newsquawk():
    ...
    while True:
        try:
            res = requests.get(NEWSQUAWK_API, timeout=30)
            headlines = res.json()
            ...
            for item in headlines:
                ...
                section = ((item.get("section") or {}).get("name") or "").strip().lower()
                ...
                if seen_ids.get(hid) != current_key:
                    ...
                    if existing_row and not content_changed:
                        ...
                        for field, label in field_map.items():
                            ...
                            current_text = (item.get(field_key) or "") if field_key else ""
...
def pre_spellcheck(text):
    """Lightweight pre-check for obvious non-dictionary words before GPT, with financial & proper noun filters"""
    words = re.findall(r"[A-Za-z']+", text)
    flagged = []

    # Finance-safe vocabulary that should NEVER be flagged
    FINANCIAL_TERMS = {
    ...
    # Merge with dynamically learned safe terms
    # Always reload latest safe terms dynamically
    FINANCIAL_TERMS |= load_safe_terms()

    for w in words:
        lw = w.lower()

        # ✅ Skip short words, approved names, or all-caps tickers
        if len(w) <= 2 or lw in APPROVED_TERMS or w.isupper():
            continue

        # ✅ Skip known financial/jargon words
        if lw in FINANCIAL_TERMS:
            continue

        # ✅ Skip proper nouns (capitalized) unless explicitly unapproved
        if w[0].isupper() and lw not in APPROVED_TERMS:
            continue

        # ❌ Flag genuine unknowns
...
def check_grammar(text, label):
    ...
    # 🔍 Pre-pass: catch clear typos before GPT (like 'frm', 'grwth', etc.)
    pre_flags = pre_spellcheck(text)
    if pre_flags:
        print(f"🚨 Pre-spellcheck caught: {pre_flags}")
        for word in pre_flags:
            text = re.sub(rf"\b{re.escape(word)}\b",
                          f"<span style='color:red'>{word}</span>", text)
        return {"highlighted": text, "corrected": text.replace("<span style='color:red'>", "").replace("</span>", "")}
    ...
    for attempt in range(3):
        ...
        print(f"⚠️ GPT 5-nano pass #{attempt + 1} returned no correction. Retrying...")
...
def check_grammar_pass(text, label, is_safety_pass=False):
    """Improved grammar checker optimized for GPT-5-nano with higher reliability for UK financial news"""
    import re
    ...
    # Use GPT-5-nano with a much more constrained prompt
...
"""
    ...
    try:
        ...
        if not highlighted_line or not corrected_line:
            print(f"⚠️ Failed to parse GPT-5-nano response for {label} - defaulting to no errors")
            clean_text = re.sub(r"<span style='color:red'>(.*?)</span>", r"\1", text)
            return {"highlighted": clean_text, "corrected": "✅ No correction needed."}
        ...
        """
                try:
                    ...
                    # ✅ Accept confirmed or short non-words as valid typos
                    if verdict == "yes" or (len(err) <= 4 and not err.isdigit() and err.lower() not in APPROVED_TERMS):
    ...
    for r in live_rows:
        if str(r.get("id")) == str(row_id):
            if section in r and "<span style='color:red'>" in r[section].get("highlighted", ""):
                # Extract the red-highlighted words
                errors = re.findall(r"<span style='color:red'>(.*?)</span>", r[section]["highlighted"])

                # Clean up the text (remove spans)
                ...
                # 💾 Save cleaned words to safe list
                for e in errors:
                    clean_term = re.sub(r"<.*?>", "", e).strip().lower()
                    if clean_term:
                        save_safe_term(clean_term)

                print(f"✅ Dismissed error in {section} for ID {row_id} | Learned safe terms: {errors}")
                break
...
def mark_corrected():
    ...
    for r in live_rows:
        if str(r.get("id")) == str(row_id):
            if section in r:
                # Stop beeping (handled client-side)
                # Just mark this section as fixed manually
                clean_text = re.sub(r"<span style='color:red'>(.*?)</span>", r"\1", r[section]["highlighted"])
                r[section]["highlighted"] = clean_text
                r[section]["corrected"] = "✅ No correction needed."
                print(f"✅ Marked corrected: ID={row_id}, section={section}")
                break
